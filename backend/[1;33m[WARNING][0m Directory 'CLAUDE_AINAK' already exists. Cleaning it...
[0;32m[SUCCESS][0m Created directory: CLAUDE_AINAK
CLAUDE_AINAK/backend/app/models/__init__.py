...
# ✅ SIMPLE MATCH RESULTS MODEL (always available)
try:
    from .match_results import MatchResult
    MATCH_RESULTS_AVAILABLE = True
    __all__.extend(["MatchResult"])
    print("✅ Match Results model imported successfully")
except ImportError as e:
    MATCH_RESULTS_AVAILABLE = False
    print(f"⚠️ Match Results model not available: {e}")
...
if CHAT_MODELS_AVAILABLE:
    available_groups.append("chat")
else:
    unavailable_groups.append("chat")
