...
class Tournament(Base):
    ...
    location = relationship(
        "Location", foreign_keys=[location_id], back_populates="tournaments"
    )
    ...
    def is_registration_open(self) -> bool:
        ...
        return (
            ...
            and self.current_participants < self.max_participants
        )
    ...
    def can_start(self) -> bool:
        ...
        return (
            self.current_participants >= self.min_participants
            and self.status
            in [TournamentStatus.REGISTRATION, TournamentStatus.REGISTRATION_CLOSED]
    ...
    def _generate_single_elimination_bracket(self) -> Dict:
        ...
        for i in range(0, participant_count, 2):
            match = {
                ...
                "player2": (
                    participants[i + 1].user_id if i + 1 < participant_count else None
                ),
                "winner": None,
                "completed": False,
            }
...
class TournamentParticipant(Base):
    ...
    __table_args__ = (
        UniqueConstraint(
            "tournament_id", "user_id", name="unique_tournament_participant"
        ),
    )
...
class TournamentMatch(Base):
    ...
    def complete_match(
        self, winner_id: int, player1_score: int, player2_score: int = None
    ):
...
class TournamentAchievement(Base):
    ...
    def __repr__(self):
        return (
            f"<TournamentAchievement(id='{self.achievement_id}', name='{self.name}')>"
        )
...
class UserTournamentAchievement(Base):
    ...
    achievement_id = Column(
        String(50), ForeignKey("tournament_achievements.achievement_id"), nullable=False
    )
