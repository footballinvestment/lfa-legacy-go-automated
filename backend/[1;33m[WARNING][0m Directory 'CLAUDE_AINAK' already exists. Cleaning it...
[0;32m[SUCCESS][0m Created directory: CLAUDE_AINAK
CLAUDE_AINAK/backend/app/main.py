...
try:
    import socketio
    from app.websocket.chat_manager import sio, socket_app
...
if websocket_available:
    ...
    logger.warning("⚠️ WebSocket components not available")
...
development_cors = [
    ...
    "http://127.0.0.1:3000",
    "http://127.0.0.1:49430"
]

allowed_origins = os.getenv("CORS_ORIGINS", ",".join(development_cors)).split(",")
...
def safe_import_router(router_path: str):
    ...
    try:
        ...
        elif router_path == "gamification":
            from app.routers.gamification import router
...
gamification_router = safe_import_router("gamification")
    ...
    if websocket_available:
        return ResponseBuilder.success(
            data={
                ...
                "status": "active",
                "features": ["real_time_chat", "room_management", "user_presence"]
            },
        ...
        return ResponseBuilder.error(
            ...
            error_message="WebSocket server is not available",
            request_id=getattr(request.state, "request_id", None)
        )
...
# WebSocket Integration - Mount after all routes
if websocket_available:
    # Mount WebSocket
