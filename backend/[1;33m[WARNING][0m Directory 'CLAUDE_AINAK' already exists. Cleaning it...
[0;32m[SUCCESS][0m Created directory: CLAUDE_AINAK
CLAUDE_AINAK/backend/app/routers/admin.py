...
async def create_user_violation(
    ...
    return {"message": "Violation created successfully", "violation": new_violation}
...
async def bulk_user_operations(
    ...
    for user_id in operation.user_ids:
        try:
            ...
            results[str(user_id)] = {"status": "success", "message": message}
            success_count += 1
...
async def get_user_reports(
    ...
    return {"reports": reports, "total": len(reports), "status_filter": status}
...
@router.get("/config")
async def get_admin_configuration(current_user: User = Depends(get_current_admin)):
