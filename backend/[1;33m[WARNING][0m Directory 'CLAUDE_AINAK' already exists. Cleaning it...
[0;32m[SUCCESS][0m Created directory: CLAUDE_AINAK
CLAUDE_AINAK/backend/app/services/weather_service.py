...
class WeatherAPIService:
    ...
    def _calculate_severity(
        self, main: Dict, weather: Dict, wind: Dict
    ) -> WeatherSeverity:
...
class WeatherService:
    ...
    async def update_location_weather(self, location_id: int) -> bool:
        ...
        try:
            ...
            weather_record = LocationWeather(location_id=location_id, **weather_data)
            ...
            logger.info(
                f"Weather updated for location {location_id}: {weather_record.condition.value} {weather_record.temperature}°C"
            )
            return True

        except Exception as e:
            logger.error(
                f"Failed to update weather for location {location_id}: {str(e)}"
            )
            self.db.rollback()
            return False
    ...
    async def update_all_locations_weather(self) -> int:
        ...
        for location in locations:
            try:
                ...
                logger.error(
                    f"Error updating weather for location {location.id}: {str(e)}"
                )
                continue
    ...
    async def get_weather_forecast(
        self, location_id: int, hours: int = 24
        ...
        try:
            forecast_data = await self.api_service.get_hourly_forecast(
                float(location.latitude), float(location.longitude), hours
            )

            forecasts = []
            for data in forecast_data:
                forecast = WeatherForecast(location_id=location_id, **data)
    ...
    def get_current_weather(self, location_id: int) -> Optional[LocationWeather]:
        ...
        return (
            self.db.query(LocationWeather)
            .filter(LocationWeather.location_id == location_id)
            .order_by(LocationWeather.updated_at.desc())
            .first()
        )

    def is_game_suitable_for_weather(
        self, game_type: str, location_id: int
    ) -> Tuple[bool, str]:
        ...
        suitability = (
            self.db.query(GameWeatherSuitability)
        ...
        if not suitability:
            ...
            is_suitable = weather.severity in [
                WeatherSeverity.LOW,
                WeatherSeverity.MODERATE,
            ]
            return (
                is_suitable,
                f"Weather severity: {weather.severity.value} (using default rules)",
            )
        ...
        try:
            ...
            for session in affected_sessions:
                ...
                if not suitable:
                    ...
                    logger.info(
                        f"Auto-cancelled session {session.session_id} due to weather: {reason}"
                    )
...
class WeatherAnalyticsService:
    ...
    def get_weather_impact_report(
        ...
        sessions = (
            self.db.query(GameSession)
            .filter(
                GameSession.start_time >= start_date, GameSession.start_time <= end_date
            )
            .all()
        )
        ...
        weather_cancelled = len(
            [
                s
                for s in sessions
                if s.status == SessionStatus.CANCELLED
                and s.cancellation_reason
                and "weather" in s.cancellation_reason.lower()
            ]
        )
        weather_warnings = len(
            [
                s
                for s in sessions
                if s.booking_notes and "weather" in s.booking_notes.lower()
            ]
        )
        ...
        return {
            ...
            "session_impact": {
                ...
                "cancellation_rate": round(
                    (
                        (weather_cancelled / total_sessions * 100)
                        if total_sessions > 0
                        else 0
                    ),
                    2,
            ...
            "revenue_impact": {
                ...
                "estimated_loss": round(
                    cancelled_revenue * 0.2, 2
                ),  # Assuming 20% don't rebook
            },
            "weather_conditions": condition_stats,
            "recommendations": self._generate_weather_recommendations(
                condition_stats, weather_cancelled, total_sessions
            ),
        }

    def _generate_weather_recommendations(
        self, condition_stats: Dict, cancelled_count: int, total_sessions: int
    ) -> List[str]:
        ...
        cancellation_rate = (
            (cancelled_count / total_sessions) if total_sessions > 0 else 0
        )

        if cancellation_rate > 0.15:  # More than 15% cancellation rate
            recommendations.append(
                "High weather cancellation rate detected. Consider implementing indoor alternatives or flexible rescheduling policies."
            )

        if (
            "heavy_rain" in condition_stats
            and condition_stats["heavy_rain"]["count"] > 5
        ):
            recommendations.append(
                "Frequent heavy rain detected. Consider covered areas or indoor facilities for rainy season."
            )

        if any(stats["avg_temp"] > 30 for stats in condition_stats.values()):
            recommendations.append(
                "High temperature periods detected. Consider early morning or evening time slots during summer."
            )

        if any(stats["avg_temp"] < 5 for stats in condition_stats.values()):
            recommendations.append(
                "Cold weather periods detected. Consider heated facilities or winter equipment."
            )

        if not recommendations:
            recommendations.append(
                "Weather impact is within normal range. Current weather policies are working well."
            )
