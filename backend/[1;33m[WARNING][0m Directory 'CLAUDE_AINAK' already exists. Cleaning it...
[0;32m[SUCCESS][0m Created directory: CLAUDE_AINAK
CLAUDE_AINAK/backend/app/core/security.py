...
HCAPTCHA_SITE_KEY = os.getenv(
    "HCAPTCHA_SITE_KEY", "3eae1014-b628-4f88-8034-cbc0d02eab34"
)
...
class SpamProtection:
    ...
    async def verify_hcaptcha(captcha_response: str, client_ip: str) -> bool:
        ...
        if not HCAPTCHA_SECRET_KEY:
            logger.warning(
                "⚠️ hCaptcha not configured - skipping verification in development"
            )
        ...
        try:
            async with httpx.AsyncClient() as client:
                ...
                if result.get("success", False):
                    ...
                    logger.warning(
                        f"❌ hCaptcha failed for IP: {client_ip} - Errors: {result.get('error-codes', [])}"
                    )
                    return False
    ...
    def check_registration_rate_limit(client_ip: str, email: str) -> bool:
        ...
        if redis_available:
            return SpamProtection._check_redis_rate_limit(
                ip_key, email_key, current_time
            )
        else:
            return SpamProtection._check_memory_rate_limit(
                ip_key, email_key, current_time
            )
    ...
    def _check_memory_rate_limit(
        ip_key: str, email_key: str, current_time: int
    ) -> bool:
    ...
    def get_rate_limit_status() -> dict:
        ...
        if redis_available:
            ...
            return {"rate_limiting": "memory", "cached_entries": len(rate_limit_cache)}
