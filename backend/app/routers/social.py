...
def get_friend_requests_for_user(user_id: int, db: Session, request_type: str = "all") -> List[Dict]:
...
def respond_to_friend_request(request_id: int, accept: bool, db: Session) -> bool:
    ...
    logger.info(f"✅ Friend request {request_id} {'accepted' if accept else 'declined'}")
    return True
...
async def get_friends(
    ...
    try:
        ...
        for friend_data in friends_data:
            win_rate = (friend_data.get("games_won", 0) / friend_data.get("games_played", 1) * 100) if friend_data.get("games_played", 0) > 0 else 0
...
async def send_challenge(
    ...
    try:
        ...
        target_user = db.query(User).filter(User.id == challenge_data.challenged_user_id).first()
        if not target_user:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND,
                detail="User not found"
        ...
        logger.info(f"✅ Challenge sent: {current_user.id} → {target_user.id} ({challenge_data.game_type})")
