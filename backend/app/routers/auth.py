...
from ..models.user import (
    ...
    UserUpdate, PasswordChange, UserLogin, UserCreateProtected
)
import os
...
async def login(user_data: UserLogin, request: Request, db: Session = Depends(get_db)):
    ...
    try:
        ...
        logger.info(f"✅ User logged in: {user.username} (IP: {client_ip}) in {time.time() - start_time:.2f}s")
...
async def register_protected(user_data: UserCreateProtected, request: Request, db: Session = Depends(get_db)):
    """🛡️ Protected user registration with enhanced security"""
    start_time = time.time()
    client_ip = request.client.host if request.client else "unknown"
    
    logger.info(f"🛡️ Protected registration attempt from IP: {client_ip}, email: {user_data.email}")
    ...
    try:
        ...
        logger.info(f"✅ Protected user registered: {new_user.username} in {time.time() - start_time:.2f}s")
...
async def register_simple(user_data: UserCreate, request: Request, db: Session = Depends(get_db)):
