...
from ..models.coupon import (
    Coupon, CouponUsage, CouponCreate, CouponRedeem, CouponInfo, 
    CouponResponse, CouponUsageHistory, CouponUsageRecord, 
    CouponStats, CouponAuditLog, CouponHealthStatus
)
...
class PurchaseRequest(BaseModel):
    package_id: str
    payment_method: str = Field(..., description="Payment method: card, paypal, bank_transfer")
...
def check_rate_limit(user_id: int, ip: str, db: Session) -> tuple[bool, str]:
    ...
    try:
        if not redis_client:
            ...
            recent_attempts = db.query(CouponUsage).filter(
                and_(
            ...
            if recent_attempts >= 5:
                return False, f"Rate limit exceeded. {recent_attempts} attempts in last hour."
            return True, "OK"
...
def validate_coupon_security(coupon_code: str, user_id: int, ip: str, db: Session) -> dict:
...
def log_coupon_attempt(user_id: int, username: str, coupon_code: str, success: bool, reason: str, ip: str, credits: int = 0):
...
def simulate_payment_processing(package: CreditPackage, payment_method: str) -> tuple[bool, str]:
...
async def purchase_credits(
    ...
    try:
        ...
        current_user.total_credits_purchased = (current_user.total_credits_purchased or 0) + total_credits
        ...
        logger.info(f"✅ Credit purchase successful: User {current_user.id} bought {total_credits} credits")
...
async def get_credit_stats(current_user: User = Depends(get_current_user)):
    ...
    try:
        transaction_count = len(current_user.transaction_history) if current_user.transaction_history else 0
...
async def get_all_transactions(
    ...
    try:
        ...
        users_with_transactions = db.query(User).filter(
            User.transaction_history.isnot(None)
...
async def redeem_coupon_secure(
    ...
    try:
        # Security validation
        validation_result = validate_coupon_security(coupon_code, current_user.id, client_ip, db)
        
        if not validation_result["valid"]:
            # Log failed attempt
            log_coupon_attempt(
                current_user.id, current_user.username, coupon_code,
                False, validation_result["reason"], client_ip
            )
        ...
        log_coupon_attempt(
            current_user.id, current_user.username, coupon_code,
            True, "Successfully redeemed", client_ip, credits_to_add
        )
        ...
        log_coupon_attempt(
            current_user.id, current_user.username, coupon_code,
            False, "Internal error", client_ip
        )
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
...
async def get_my_coupon_usage_secure(
    ...
    try:
        ...
        usage_records = db.query(CouponUsage).filter(
            CouponUsage.user_id == current_user.id
        ).order_by(CouponUsage.redeemed_at.desc()).all()
...
async def get_coupon_statistics(
    ...
    try:
        ...
        today_redemptions = db.query(CouponUsage).filter(
            CouponUsage.redeemed_at >= today_start
        ).count()
        ...
        popular_coupon = db.query(Coupon.code, func.count(CouponUsage.id)).join(
            CouponUsage, Coupon.id == CouponUsage.coupon_id
        ).group_by(Coupon.code).order_by(func.count(CouponUsage.id).desc()).first()
...
async def create_coupon_secure(
    ...
    try:
        ...
        logger.info(f"✅ Admin {current_user.username} created coupon '{new_coupon.code}'")
...
async def get_coupon_audit_log(
    ...
    try:
        ...
        audit_records = db.query(CouponUsage).join(
            User, CouponUsage.user_id == User.id
        ).order_by(CouponUsage.redeemed_at.desc()).limit(limit).all()
...
async def detect_coupon_abuse(
    ...
    try:
        ...
        ).join(User, CouponUsage.user_id == User.id).filter(
            CouponUsage.redeemed_at >= one_day_ago
        ).group_by(CouponUsage.user_id, User.username).having(
            func.count(CouponUsage.id) > 3  # More than 3 coupons in 24h
        ).all()
        ...
        suspicious_ips = db.query(
            ...
            CouponUsage.redeemed_at >= one_day_ago,
            CouponUsage.user_ip.isnot(None)
        ).group_by(CouponUsage.user_ip).having(
            func.count(func.distinct(CouponUsage.user_id)) > 2  # More than 2 users per IP
        ).all()
