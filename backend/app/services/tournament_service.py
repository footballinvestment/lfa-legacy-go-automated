...
class TournamentService:
    ...
    def get_tournament_participants(self, tournament_id: int) -> List[TournamentParticipant]:
        ...
        return self.db.query(TournamentParticipant).filter(
    ...
    def get_user_tournaments(self, user_id: int, status: Optional[str] = None) -> List[Tournament]:
        """Get tournaments user is participating in"""
        query = self.db.query(Tournament).join(TournamentParticipant).filter(
            and_(
                TournamentParticipant.user_id == user_id,
                TournamentParticipant.status != ParticipantStatus.WITHDREW
            )
    ...
    def _validate_tournament_data(self, data: Dict):
        ...
        required_fields = ["name", "tournament_type", "game_type", "format", 
                          "location_id", "start_time", "end_time", "registration_deadline", 
                          "max_participants"]
    ...
    def _is_location_available(self, location_id: int, start_time: datetime, end_time: datetime) -> bool:
        ...
        try:
            ...
            conflicts = self.db.query(Tournament).filter(
                and_(
                    Tournament.location_id == location_id,
                    Tournament.status.in_([TournamentStatus.REGISTRATION, TournamentStatus.REGISTRATION_CLOSED, TournamentStatus.IN_PROGRESS]),
...
class TournamentLifecycleManager:
    ...
    def cancel_tournament(self, tournament_id: int, reason: str) -> bool:
        ...
        try:
            ...
            participants = self.db.query(TournamentParticipant).filter(
                and_(
                    TournamentParticipant.tournament_id == tournament_id,
                    TournamentParticipant.status.in_([ParticipantStatus.REGISTERED, ParticipantStatus.CONFIRMED])
                )
            ).all()
    ...
    def complete_tournament(self, tournament_id: int, winner_id: int, final_standings: List[Dict]) -> bool:
    ...
    def submit_match_result(self, match_id: int, winner_id: int, player1_score: int, player2_score: int, submitter_id: int) -> bool:
        """Submit match result"""
        try:
            match = self.db.query(TournamentMatch).filter(TournamentMatch.id == match_id).first()
...
class TournamentAnalyticsService:
    ...
    def get_tournament_stats(self, tournament_id: int) -> Dict:
        ...
        return {
            ...
            "active_participants": len([p for p in participants if p.status == ParticipantStatus.REGISTERED]),
            "total_prize_pool": tournament.prize_pool_credits or 0,
            "average_level": sum(p.user.level for p in participants) / len(participants) if participants else 0,
            "completion_rate": 100.0 if tournament.status == TournamentStatus.COMPLETED else 0.0
        }
...
class SingleEliminationService:
    ...
    def _create_bracket_structure(self, participants: List[TournamentParticipant]) -> Dict:
        """Create bracket structure"""
        ...
        bracket = {
            "participants": [{"user_id": p.user_id, "seed": i+1} for i, p in enumerate(shuffled)],
            "rounds": [],
            "matches": []
        }
...
class TournamentTemplateService:
    ...
    def create_daily_tournament(self, location_id: int, game_type: str) -> Tournament:
        ...
        start_time = datetime.now().replace(hour=18, minute=0, second=0, microsecond=0) + timedelta(days=1)
        ...
        return tournament_service.create_tournament(tournament_data, organizer_id=1)  # System organizer
    ...
    def create_weekly_cup(self, location_id: int, game_type: str) -> Tournament:
        ...
        start_time = (now + timedelta(days=days_ahead)).replace(hour=14, minute=0, second=0, microsecond=0)
