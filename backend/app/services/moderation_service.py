...
from ..schemas.moderation import (
    ...
    ModerationLogCreate, BulkUserOperation, BulkOperationResult,
    UserReportCreate, UserReportUpdate,
...
class ModerationService:
    ...
    def create_violation(self, user_id: int, violation_data: ViolationCreate) -> ViolationResponse:
        """Create a new user violation"""
        try:
            self.logger.info(f"Creating violation for user {user_id}, type: {violation_data.type.value}")
            ...
            self.logger.info(f"Successfully created violation {violation.id} for user {user_id}")
            return ViolationResponse.model_validate(violation)
            
        except Exception as e:
            self.logger.error(f"Failed to create violation for user {user_id}: {str(e)}")
            self.db.rollback()
            raise
    ...
    def get_user_violations(
        ...
        total = query.count()
        violations = query.order_by(desc(UserViolation.created_at))\
            .offset((page - 1) * limit)\
            .limit(limit)\
            .all()
    ...
    def update_violation(
        ...
        """Update a violation"""
        violation = self.db.query(UserViolation).filter(
    ...
    def update_user(self, user_id: int, user_data: AdminUserUpdate, actor_id: int) -> Optional[AdminUserResponse]:
    ...
    # Bulk operations
    def perform_bulk_operation(self, operation: BulkUserOperation, actor_id: int) -> BulkOperationResult:
        """Perform bulk operations on multiple users"""
        self.logger.info(f"Starting bulk operation {operation.action} on {len(operation.user_ids)} users by actor {actor_id}")
        ...
        for user_id in operation.user_ids:
            try:
                ...
                if operation.action == "suspend":
                    ...
                    results[str(user_id)] = {"status": "ok", "message": "User suspended successfully"}
                    success_count += 1

                elif operation.action == "unsuspend":
                    self._unsuspend_user(user_id, actor_id)
                    results[str(user_id)] = {"status": "ok", "message": "User unsuspended successfully"}
                    success_count += 1

                elif operation.action == "ban":
                    self._ban_user(user_id, actor_id, operation.params)
                    results[str(user_id)] = {"status": "ok", "message": "User banned successfully"}
                    success_count += 1

                else:
                    error_msg = f"Unknown action: {operation.action}"
                    self.logger.warning(f"Invalid bulk operation action: {operation.action} for user {user_id}")
        ...
        self.logger.info(f"Bulk operation completed: {success_count} success, {error_count} failures")
    ...
    def get_moderation_logs(
        ...
        total = query.count()
        logs = query.order_by(desc(ModerationLog.created_at))\
            .offset((page - 1) * limit)\
            .limit(limit)\
            .all()
    ...
    def update_report(
        ...
        if action == "dismiss":
            ...
            report.resolution_notes = data.get("notes", "Report dismissed by admin") if data else "Report dismissed by admin"
            ...
            report.assigned_to = actor_id
            report.resolution_notes = data.get("notes", "Escalated for further review") if data else "Escalated for further review"
    ...
    def _suspend_user(self, user_id: int, actor_id: int, params: Optional[Dict[str, Any]]):
