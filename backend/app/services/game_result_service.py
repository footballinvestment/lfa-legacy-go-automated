...
class GameResultService:
    ...
    def record_game_result(
        ...
        try:
            ...
            if session.status != "completed":
                raise ValueError(f"Cannot record results for session with status: {session.status}")
            
            # Get game definition for XP calculation
            game_definition = self.db.query(GameDefinition).filter(
                GameDefinition.id == session.game_definition_id
            ).first()
            ...
            existing_result = self.db.query(GameResult).filter(
                GameResult.session_id == session_id,
                GameResult.user_id == user_id
            ).first()
            
            if existing_result:
                raise ValueError(f"Result already recorded for user {user_id} in session {session_id}")
            
            # Calculate performance percentage
            final_score = float(result_data.get("final_score", 0))
            max_possible = float(result_data.get("max_possible_score", 100))
            performance_percentage = (final_score / max_possible) * 100 if max_possible > 0 else 0
            ...
            logger.info(f"Game result recorded: Session {session_id}, User {user_id}, Score {final_score}, XP {xp_earned}")
    ...
    def update_game_result(
        ...
        if 'final_score' in update_data or 'max_possible_score' in update_data:
            result.performance_percentage = (result.final_score / result.max_possible_score) * 100
    ...
    def get_user_results(
        ...
        if game_type:
            query = query.join(GameSession).join(GameDefinition).filter(
                GameDefinition.game_type == game_type
            )
        
        results = query.order_by(desc(GameResult.game_completed_at)).offset(offset).limit(limit).all()
    ...
    def get_session_results(self, session_id: str) -> List[GameResult]:
        ...
        """
        return self.db.query(GameResult).filter(
            GameResult.session_id == session_id
        ).order_by(desc(GameResult.final_score)).all()
    ...
    def _generate_category_leaderboard(self, category: str, metric: str):
        ...
        if category == "overall":
            if metric == "average_score":
                query = self.db.query(PlayerStatistics, User).join(User).filter(
                    PlayerStatistics.total_games_completed >= 5
                ).order_by(desc(PlayerStatistics.average_score)).limit(100)
            else:  # total_xp_earned_from_games
                query = self.db.query(PlayerStatistics, User).join(User).order_by(
                    desc(PlayerStatistics.total_xp_earned_from_games)
                ).limit(100)
        else:
            ...
            if category.startswith("game"):
                ...
                query = self.db.query(PlayerStatistics, User).join(User).filter(
                    func.json_extract(getattr(PlayerStatistics, game_field), "$.games_played") >= 3
                ).order_by(
                    desc(func.json_extract(getattr(PlayerStatistics, game_field), "$.avg_score"))
                ).limit(50)
            else:  # skill-specific
                skill_field = f"{category}_average"
                query = self.db.query(PlayerStatistics, User).join(User).filter(
                    getattr(PlayerStatistics, skill_field) > 0
                ).order_by(desc(getattr(PlayerStatistics, skill_field))).limit(50)
    ...
    def get_leaderboard(
        ...
        return self.db.query(Leaderboard).filter(
            ...
            Leaderboard.is_current == True
        ).order_by(asc(Leaderboard.rank)).limit(limit).all()
    
    def get_user_rank(self, user_id: int, category: str = "overall", period: str = "all_time") -> Optional[int]:
        """
    ...
    def get_performance_analytics(
        ...
        # Skill analytics
        skill_scores = {}
        for skill in ['accuracy_score', 'speed_score', 'technique_score', 'consistency_score']:
            scores = [getattr(r, skill) for r in results if getattr(r, skill) is not None]
            if scores:
...
class LeaderboardService:
    ...
    def get_top_performers(
        ...
        """
        leaderboard_entries = self.db.query(Leaderboard, User).join(User).filter(
            Leaderboard.category == category,
            Leaderboard.period == "all_time",
            Leaderboard.is_current == True
        ).order_by(asc(Leaderboard.rank)).limit(limit).all()
        ...
        for entry, user in leaderboard_entries:
            ...
            recent_games = self.db.query(GameResult).filter(
                GameResult.user_id == user.id,
                GameResult.game_completed_at >= datetime.utcnow() - timedelta(days=30)
            ).count()
    ...
    def get_user_leaderboard_position(self, user_id: int) -> Dict:
        ...
        for category in categories:
            ...
            if entry:
                ...
                total_participants = self.db.query(Leaderboard).filter(
                    Leaderboard.category == category,
                    Leaderboard.period == "all_time",
                    Leaderboard.is_current == True
                ).count()
                ...
                positions[category] = {
                    ...
                    "percentile": round((1 - (entry.rank - 1) / total_participants) * 100, 1)
                }
