...
class EnhancedBookingService:
    ...
    async def create_booking_with_weather_check(
        ...
        try:
            ...
            game_type = booking_data.get("game_type", "GAME1")  # Default to GAME1
            start_time = datetime.fromisoformat(booking_data["start_time"])
            
            # Get location and game definition
            location = self.db.query(Location).filter(Location.id == location_id).first()
            if not location:
                return False, "Location not found", None
            
            game_definition = self.db.query(GameDefinition).filter(
                GameDefinition.game_id == game_type
            ).first()
            if not game_definition:
                return False, f"Game type {game_type} not found", None
            
            # Check current weather suitability
            suitable, reason = self.weather_service.is_game_suitable_for_weather(game_type, location_id)
            if not suitable:
                return False, f"Weather conditions not suitable: {reason}", None
            
            # Check forecast for booking time (if booking is more than 1 hour ahead)
            time_until_game = start_time - datetime.utcnow()
            weather_warning = None
            
            if time_until_game.total_seconds() > 3600:  # More than 1 hour ahead
                # Get weather forecast for booking time
                forecasts = await self.weather_service.get_weather_forecast(location_id, 48)
                booking_forecast = None
                
                for forecast in forecasts:
                    if abs((forecast.forecast_time - start_time).total_seconds()) < 3600:  # Within 1 hour
                        booking_forecast = forecast
                        break
                
                if booking_forecast and booking_forecast.severity == WeatherSeverity.EXTREME:
                    return False, f"Extreme weather forecast for game time: {booking_forecast.condition.value}", None
                elif booking_forecast and booking_forecast.severity == WeatherSeverity.HIGH:
                    weather_warning = f"High severity weather forecast: {booking_forecast.condition.value}"
            ...
            if user.credits < game_definition.credit_cost:
                return False, f"Insufficient credits. Required: {game_definition.credit_cost}, Available: {user.credits}", None
            
            # Create booking session
            session = await self._create_booking_session(booking_data, user_id, game_definition, location)
            
            # Add weather warning if present
            current_weather = self.weather_service.get_current_weather(location_id)
            if current_weather and current_weather.severity in [WeatherSeverity.MODERATE, WeatherSeverity.HIGH]:
    ...
    async def _create_booking_session(
        ...
        session_id = f"LFA_{datetime.now().strftime('%Y%m%d')}_{str(uuid.uuid4())[:8].upper()}"
        ...
        if game_definition.required_equipment:
            ...
            for set_name in location.equipment_sets.keys():
                if location.reserve_equipment_set(set_name):  # JAVÍTÁS: reserve_equipment_set()
    ...
    async def check_upcoming_bookings_weather(self):
        ...
        try:
            ...
            upcoming_sessions = self.db.query(GameSession).filter(
            ...
            for session in upcoming_sessions:
                ...
                if not suitable:
                    ...
                    current_weather = self.weather_service.get_current_weather(session.location_id)
                    
                    if current_weather and current_weather.severity == WeatherSeverity.EXTREME:
            ...
            logger.info(f"Weather check completed: {processed_count} sessions processed, {warned_count} warned, {cancelled_count} cancelled")
            return {
    ...
    async def _auto_cancel_session(self, session: GameSession, reason: str):
        ...
        try:
            ...
            if session.equipment_set and session.reserved_equipment:
                session.location.release_equipment_set(session.equipment_set)  # JAVÍTÁS: release_equipment_set()
            
            self.db.commit()
            logger.info(f"Auto-cancelled session {session.session_id} due to extreme weather: {reason}")
            
            # TODO: Send notifications to all players
            # notification_service.send_weather_cancellation(session, reason)
            
        except Exception as e:
            logger.error(f"Error auto-cancelling session {session.session_id}: {str(e)}")
            self.db.rollback()
    ...
    async def _create_session_weather_alert(self, session: GameSession, reason: str):
        ...
        try:
            ...
            self.db.commit()
            logger.info(f"Weather alert created for session {session.session_id}: {reason}")
            
        except Exception as e:
            logger.error(f"Error creating weather alert for session {session.session_id}: {str(e)}")
            self.db.rollback()
    
    def cancel_booking_with_weather_refund(self, session_id: str, user_id: int, reason: str) -> Tuple[bool, str, float]:
        ...
        try:
            ...
            if session.booked_by != user_id and not any(p.get("user_id") == user_id for p in session.players):
            ...
            # Calculate refund based on weather and timing
            can_cancel, cancel_reason, base_refund_percentage = session.can_be_cancelled()
            if not can_cancel:
            ...
            if "weather" in reason.lower():
                ...
                if current_weather:
                    if current_weather.severity == WeatherSeverity.EXTREME:
                        weather_bonus = 1.0 - base_refund_percentage  # Full refund for extreme weather
                    elif current_weather.severity == WeatherSeverity.HIGH:
                        weather_bonus = 0.25  # 25% bonus for high severity
            ...
            if user and refund_amount > 0:
                ...
                refund_transaction = {
                    ...
                    "refund_percentage": final_refund_percentage,
            ...
            self.db.commit()
            return True, f"Session cancelled successfully. Refund: {refund_amount} credits ({int(final_refund_percentage*100)}%)", final_refund_percentage
    ...
    def get_weather_suitable_time_slots(self, location_id: int, game_type: str, date: datetime) -> List[Dict]:
            ...
            for slot_time in base_slots:
                ...
                if slot_time.date() == datetime.now().date():
                    ...
                    slot_info = {
                        ...
                        "reason": reason,
                        "weather": {
                            "condition": current_weather.condition.value if current_weather else "unknown",
                            "temperature": current_weather.temperature if current_weather else None,
                            "severity": current_weather.severity.value if current_weather else "unknown"
                        }
                    }
                else:
    ...
    def get_booking_weather_summary(self, session_id: str) -> Dict:
        ...
        try:
            ...
            active_alerts = location.get_active_weather_alerts() if hasattr(location, 'get_active_weather_alerts') else []
