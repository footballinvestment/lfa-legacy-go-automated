...
class ModerationFilter(logging.Filter):
    ...
    def filter(self, record: logging.LogRecord) -> bool:
        ...
        return (
            record.name in moderation_modules or 
            hasattr(record, 'action') or
            hasattr(record, 'actor_id')
        )
...
def setup_logging(
    ...
    loggers = {
        "app.services.moderation_service": {
            "level": log_level,
            "handlers": ["file", "moderation_file"] + (["console"] if enable_console else []),
            "propagate": False
        },
        "app.routers.admin": {
            "level": log_level,
    ...
    logger.info(f"Logging configured: level={log_level}, format={log_format}, file={log_file_path}")
