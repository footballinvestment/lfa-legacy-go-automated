...
def log_api_call(endpoint: str, method: str, user_id: Optional[int] = None, 
    ...
    logger.info(
        ...
        f"response_time={response_time:.3f}s" if response_time else f"response_time=None"
    )
...
def get_system_health() -> Dict[str, Any]:
    ...
    return {
        ...
        "error_monitor": {
            ...
            "recent_error_rate": len(error_monitor.error_log) / max(1, 60)  # errors per minute
        },
        "performance": {
            "monitored_operations": len(error_monitor.performance_stats),
            "total_operations": sum(stats["count"] for stats in error_monitor.performance_stats.values()),
            "average_response_time": sum(
...
def check_error_monitor_health() -> Dict[str, Any]:
    ...
    slow_operations = [
        op for op, stats in error_monitor.performance_stats.items()
