...
class Coupon(Base):
    ...
    def can_be_used_by_user(self, user_id: int, db_session) -> tuple[bool, str]:
        ...
        existing_usage = (
            db_session.query(CouponUsage)
            .filter(
                CouponUsage.user_id == user_id, CouponUsage.coupon_code == self.code
            )
            .first()
        )
        ...
        if existing_usage:
            return (
                False,
                f"Coupon already redeemed on {existing_usage.redeemed_at.strftime('%Y-%m-%d %H:%M')}",
            )
