...
class FriendRequest(Base):
    ...
    def get_pending_requests_for_user(
        ...
        return (
            db.query(cls)
            .filter(
                cls.receiver_id == user_id, cls.status == FriendRequestStatus.PENDING
            )
            .all()
        )

    @classmethod
    def get_sent_requests_by_user(
        cls, db: Session, user_id: int
    ) -> List["FriendRequest"]:
        """Get all friend requests sent by a user"""
        return (
            db.query(cls)
            .filter(cls.sender_id == user_id, cls.status == FriendRequestStatus.PENDING)
            .all()
        )
...
class Friendship(Base):
    ...
    def create_friendship(
        cls, db: Session, user1_id: int, user2_id: int
    ) -> "Friendship":
        ...
        friendship = cls(user1_id=user1_id, user2_id=user2_id, status="active")
        db.add(friendship)
        db.commit()
        return friendship
...
class UserBlock(Base):
    ...
    def get_blocked_users(cls, db: Session, blocker_id: int) -> List["UserBlock"]:
        ...
        return (
            db.query(cls)
            .filter(cls.blocker_id == blocker_id, cls.is_active.is_(True))
            .all()
        )
...
def create_friendship_from_request(
    db: Session, friend_request: FriendRequest
) -> Friendship:
    """Create a friendship from an accepted friend request"""
    friendship = Friendship.create_friendship(
        db=db, user1_id=friend_request.sender_id, user2_id=friend_request.receiver_id
    )
    return friendship
...
def get_friendship_between_users(
...
def get_mutual_friends(db: Session, user1_id: int, user2_id: int) -> List[int]:
    ...
    user1_friends = [
        f.get_friend_of(user1_id) for f in Friendship.get_friends_of_user(db, user1_id)
    ]
    user2_friends = [
        f.get_friend_of(user2_id) for f in Friendship.get_friends_of_user(db, user2_id)
    ]
...
def can_interact(db: Session, user1_id: int, user2_id: int) -> bool:
    ...
    return not (
        UserBlock.is_blocked(db, user1_id, user2_id)
        or UserBlock.is_blocked(db, user2_id, user1_id)
    )


def create_friend_request(
    db: Session, sender_id: int, receiver_id: int, message: str = None
) -> FriendRequest:
...
def respond_to_friend_request(
    db: Session, request_id: int, action: str, user_id: int
) -> dict:
