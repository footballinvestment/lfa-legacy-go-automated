...
class GameResult(Base):
    ...
    percentage_score = Column(Float, nullable=False, index=True)  # final_score / max_possible_score * 100
    ...
    percentile_rank = Column(Float, nullable=True)  # Player's percentile among all players
    level_percentile = Column(Float, nullable=True)  # Percentile among same-level players
...
class Leaderboard(Base):
    ...
    category = Column(String(50), nullable=False, index=True)  # overall, game1, game2, etc.
    time_period = Column(String(20), nullable=False, index=True)  # all_time, monthly, weekly, daily
    ...
    __table_args__ = (
        UniqueConstraint('leaderboard_id', 'player_id', name='unique_leaderboard_entry'),
...
class PlayerAchievement(Base):
    ...
    id = Column(Integer, primary_key=True, index=True)
    player_id = Column(Integer, ForeignKey("users.id"), nullable=False, index=True)
    achievement_id = Column(String(50), ForeignKey("game_achievements.achievement_id"), nullable=False)
    game_result_id = Column(String(50), ForeignKey("game_results.result_id"), nullable=True)
