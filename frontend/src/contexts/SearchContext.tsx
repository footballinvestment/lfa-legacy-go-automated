...
export const SearchProvider: React.FC<SearchProviderProps> = ({ children }) => {
  // Core search state
  const [criteria, setCriteriaState] = useState<SearchCriteria>(defaultSearchCriteria);
  ...
  const [filterBuilder, setFilterBuilder] = useState<FilterBuilderGroup | null>(null);
  ...
  // Update category
  const updateCategory = useCallback((category: SearchCategory) => {
  ...
  const executeSearch = useCallback(async () => {
    ...
    try {
      ...
      const mockResults: SearchResponse = {
        ...
        suggestions: ['football tournament', 'local matches', 'beginner friendly'],
        executionTime: 156,
  ...
  const generateMockResults = (criteria: SearchCriteria): SearchResult[] => {
    const baseResults: SearchResult[] = [
      ...
      },
      {
        ...
        description: 'Professional-grade football fields with modern facilities',
        thumbnail: '/api/placeholder/100/100',
        url: '/locations/3',
        metadata: {
          type: 'complex',
  ...
  const getSuggestions = useCallback(async (query: string): Promise<SearchSuggestion[]> => {
    ...
    const mockSuggestions: SearchSuggestion[] = [
      ...
      { text: 'beginner friendly', type: 'filter' as const, category: 'tournaments' as SearchCategory, popularity: 65 },
      { text: 'tournaments', type: 'category' as const, category: 'tournaments' as SearchCategory, popularity: 90 }
