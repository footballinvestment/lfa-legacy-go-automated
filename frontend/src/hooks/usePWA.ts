...
const usePWA = () => {
  ...
  const [notificationPermission, setNotificationPermission] = useState<NotificationPermission['status']>('default');
  ...
  const checkCapabilities = useCallback(() => {
    ...
    newCapabilities.canInstall = installPrompt !== null && !newCapabilities.isInstalled;
  ...
  const requestNotificationPermission = useCallback(async (): Promise<NotificationPermission['status']> => {
    if (!('Notification' in window)) {
      return 'denied';
    }
  ...
  const registerBackgroundSync = useCallback(async (tag: string): Promise<boolean> => {
    if (!capabilities.supportsBackgroundSync || !capabilities.hasServiceWorker) {
      return false;
    }
  ...
  const subscribeToPushNotifications = useCallback(async (
    ...
    if (!capabilities.supportsNotifications || !capabilities.hasServiceWorker) {
      return null;
    }
