...
const useTouchGestures = (
  ...
  const handleTouchStart = useCallback((event: TouchEvent) => {
    ...
    if (enabledGestures.includes('longPress') && event.touches.length === 1) {
      longPressTimerRef.current = setTimeout(() => {
        const gestureEvent = createGestureEvent('longPress', undefined, event.target);
  ...
  const handleTouchEnd = useCallback((event: TouchEvent) => {
    ...
    // Handle tap gesture
    if (enabledGestures.includes('tap') && distance < tapThreshold && duration < 300) {
    ...
    // Handle swipe gestures
      ...
      if (handler) {
        const gestureEvent = createGestureEvent('swipe', direction, event.target);
