...
describe("AdvancedModerationTools", () => {
  ...
  it("displays reports list with correct data", async () => {
    ...
    await waitFor(() => {
      expect(
        screen.getByText("User is being inappropriate")
      ).toBeInTheDocument();
      expect(
        screen.getByText("Suspected cheating in tournament")
      ).toBeInTheDocument();
  ...
  it("handles report action submission", async () => {
    ...
    fireEvent.change(notesField, {
      target: { value: "False positive report" },
    });
    ...
    await waitFor(() => {
      expect(mockUpdateReport).toHaveBeenCalledWith(1, "dismiss", {
  ...
  it("handles settings toggle switches", async () => {
    ...
    await waitFor(() => {
      const autoAssignSwitch = screen.getByRole("checkbox", {
        name: /auto-assign reports/i,
      });
