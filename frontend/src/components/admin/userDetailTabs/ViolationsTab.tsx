...
const ViolationsTab: React.FC<ViolationsTabProps> = ({ user }) => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      if (editingViolation) {
        await moderationApi.updateViolation(user.id, editingViolation.id, formData);
        setSnackbar({
          open: true,
      ...
      setSnackbar({
        open: true,
        message: error instanceof Error ? error.message : 'Failed to save violation',
        severity: 'error',
      });
    }
  };
  ...
  const getViolationSeverityColor = (type: string) => {
    switch (type) {
      ...
      case 'suspension': case 'cheating': return 'error';
      case 'harassment': case 'inappropriate_conduct': return 'error';
      case 'spam': case 'terms_violation': return 'warning';
                ...
                <TableRow key={violation.id}>
                  ...
                  <TableCell>
                    ...
                    <Tooltip title="Delete">
                      <IconButton size="small" onClick={() => handleDelete(violation.id)}>
