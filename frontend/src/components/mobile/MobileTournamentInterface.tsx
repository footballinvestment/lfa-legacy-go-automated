...
const MobileTournamentInterface: React.FC = () => {
  ...
  const handleShareTournament = async (tournament: Tournament) => {
    if (navigator.share) {
      ...
      navigator.clipboard.writeText(
        `Check out this tournament: ${tournament.name} - ${window.location.href}/tournament/${tournament.id}`
      );
  ...
  const handleTabChange = (_: React.SyntheticEvent, newValue: number) => {
    ...
    const filters: Array<"all" | "upcoming" | "active" | "completed"> = [
      "all",
      "upcoming",
      "active",
      "completed",
            ...
            {selectedTournament && (
              <Box>
                ...
                <Box display="flex" gap={2} mb={2}>
                  ...
                  <Chip
                    label={`Prize: $${selectedTournament.prizePool}`}
                    color="primary"
                  />
                </Box>
                <Alert severity="info">
                  By joining this tournament, you agree to pay the entry fee and
